...
namespace pidgeon_sv
{
    public class SecuredListener : Listener
    {
        ...
        /// Port.
        /// </param>
        public SecuredListener(int port)
        {
            this.Port = port;
        }
        ...
        private void Exec()
        {
            while (this.IsRunning)
            {
                try
                {
                    ...
                    _client.Name = "Listener/SSL:" + connection.Client.RemoteEndPoint.ToString();
                ...
                {
                    SystemLog.WriteLine("Listener for services (encrypted connection on port: " +
                           this.Port + ") is: DOWN");
                    return;
                }
        ...
        public override bool Listen()
        {
            try
            {
                SystemLog.WriteLine("Opening listener for services (encrypted connection on port: " +
                               this.Port + ")");
